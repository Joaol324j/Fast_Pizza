...
def adicionar_promocao(promocao: PromocaoCreate, db: Session = Depends(get_db)):
    return PromocaoService.adicionar_promocao(db, promocao)

@router.put("/{promocao_id}", response_model=PromocaoResponse)
def atualizar_promocao(promocao_id: int, promocao: PromocaoCreate, db: Session = Depends(get_db)):
...
def remover_promocao(promocao_id: int, db: Session = Depends(get_db)):
